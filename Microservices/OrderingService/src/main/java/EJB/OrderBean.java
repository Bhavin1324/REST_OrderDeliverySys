...
//        Users user = (Users) em.find(Users.class,userId);
    ...
    public JsonObject getOrdersByDateAndOutlet(String outletid) {
        try {
            ...
            List<OrderMaster> todaysOrder = (List<OrderMaster>) em.createQuery("SELECT o FROM OrderMaster o WHERE o.outletId.id =:outletid AND o.orderDate = :orderDate AND o.orderStatus!= :status").setParameter("outletid", outletid).setParameter("orderDate", new Date()).setParameter("status", OrderStatus.PREPARING.toString()).getResultList();
