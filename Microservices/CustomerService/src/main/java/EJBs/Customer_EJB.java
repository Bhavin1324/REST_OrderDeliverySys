...
//    EntityManager em = emf.createEntityManager();
    ...
    public PHResponseType register(JsonObject data) {
        ...
        try {
            ...
            double credits = (double) data.getInt("credits");
            //user details
    ...
    public JsonObject login(JsonObject data) {
        ...
        try {
            ...
            if(role.equals(Constants.ROLE_DELIVERY_PERSON)){
                DeliveryPerson person = (DeliveryPerson) em.createQuery("SELECT d FROM DeliveryPerson d WHERE d.username.username = :username").setParameter("username", user.getUsername()).getSingleResult();
                userid=person.getId();
            }
            Pbkdf2PasswordHashImpl pbk = new Pbkdf2PasswordHashImpl();
